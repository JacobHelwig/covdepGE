...
ELBO_calculator_R <- function(y, D, X, ssq_var, mu, alpha, ssq, sbsq, pip) {
  ...
  eqpr <- sum(-alpha / 2 * log(ssq * sbsq) - alpha * (ssq_var + mu_sq) /
               (2 * ssq * sbsq) + alpha * log(pip) + (1 - alpha) * log(1 - pip))

  # calculate the expected value of the log-likelihood under q
  eqlik <- -sum(
    D * ((y - (X %*% (mu * alpha)))^2 + X^2 %*% (alpha * (
      mu_sq + ssq_var) - alpha^2 * mu_sq))) / (2 * ssq) - nrow(X) / 2 *
      log(2 * pi * ssq)

  # calculate the expected value of log q under q
  eqq = sum(-alpha / 2 * log(ssq_var) - alpha / 2 + alpha * log(alpha + 0.000001) +
              (1 - alpha) * log(1 - alpha + 0.000001))
