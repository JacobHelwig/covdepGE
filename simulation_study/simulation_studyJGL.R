...
JGL.eval <- function(X, Z, true){
  ...
  for(k in 1:length(lambda1)){
    ...
    aic_lambda1[[k]] <- list(lambda = lambda1[k], aic = aic_JGL(X_k, out$theta), time = time)
  }
  ...
  for (k in 1:length(lambda2)){
    ...
    out <- JGL(Y = X_k,
               ...
               return.whole.theta = T)
    time <- as.numeric(Sys.time() - start, units = "secs")
    aic_lambda2[[k]] <- list(lambda = lambda2[k], aic = aic_JGL(X_k, out$theta), time = time)
  }
...
for (j in 1:n_trials){
  ...
  save(results, file = paste0("resJGL_p", p, "_n", n, "_", now, ".Rda"))
