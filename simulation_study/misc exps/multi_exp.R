...
generateData <- function(p = 5, n1 = 60, n2 = 60, n3 = 60){
  ...
  # draw the covariate values within each interval
  z1_2 <- sort(stats::runif(n1, limits1[1], limits1[2]), decreasing = T)
  z2_2 <- sort(stats::runif(n2, limits2[1], limits2[2]), decreasing = T)
  z3_2 <- sort(stats::runif(n3, limits3[1], limits3[2]), decreasing = T)
  Z2 <- matrix(c(z3_2, z2_2, z1_2), n, 1)
  ...
  str12 <- str13 <- str24 <- matrix(0, p, p)
  str12[1, 2] <- str13[1, 3] <- str24[2, 4] <- 1
  ...
  return(list(X = data_mat, Z = cbind(Z, Z2), true_precision = true_precision,
              interval = interval))
}
...
tvmgm.eval <- function(X, Z, true){
  ...
  for (j in 2:nrow(Z)){
    ...
    # get the last index and corresponding Z
    last_ind <- Z_sorted[j - 1]
    Z_last <- matrix(Z[last_ind, ], nrow(Z_rem), 2, T)

    # get norm with all the remaining and find the minimizer
    min_ind <- which.min(rowSums((Z_rem - Z_last)^2))

    # add the min ind to the sorted Z and mask out the corresponding Z from the
    # remaining Z
    Z_sorted <- c(Z_sorted, min_ind)
    Z_rem[min_ind, ] <- Inf
  }
