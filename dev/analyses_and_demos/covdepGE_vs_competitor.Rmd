...
sigmasq <- sapply(1:(p + 1), function (col_ind) mean(varbvs::varbvs(data_mat[ , -col_ind], Z, data_mat[ , col_ind], verbose = F)$sigma))
sigmasq
mean(sigmasq)
```
```{r}
...
annotate_figure(ggarrange(plotlist = gg_adjMats(out_dep, colors)), top = text_grob("Covariate Dependent Estimated Graphs", size = 15))
```
...
for (k in 1:num_clusters){
  ...
  # use varbvs to get the hyperparameter sigma
  sigmasq_k <- sapply(1:(p + 1), function (col_ind) mean(varbvs::varbvs(data_mat_k[ , -col_ind], NULL, data_mat_k[ , col_ind], verbose = F)$sigma))
...
if(length(pip_mats) != num_clusters | length(adj_mats) != num_clusters){
  stop("Too many graphs")
}
...
# create a list of n graphs according to the independent estimate for each cluster and the gmm cluster assignment for each individual
...
annotate_figure(ggarrange(gg_inclusionCurve(out_dep, 1, 2), gg_inclusionCurve(out_dep, 1, 3, point_color = "dodgerblue")), top = text_grob("Posterior Inclusion Probabilties, Dependent Model", size = 15))
