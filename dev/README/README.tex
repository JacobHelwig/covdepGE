...
\item Add \texttt{KDE} argument (boolean). Then, the argument \texttt{tau} is only used when \texttt{KDE = F}
\item Allow for user specification of a norm (\texttt{"1", "2", }or \texttt{"inf"}) and symmetrization method (\texttt{"mean", "min", }or \texttt{"max"})
\item Allow for user specification of hyperparameter \texttt{sigmasq}
...
\item Add warnings for when the optimal \texttt{sigmabeta\_sq} is chosen at either of the grid end points
