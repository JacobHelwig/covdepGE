...
\begin{itemize}
\item Topic 16 slide ideas  
\item 2 ideas from `Mclust`: logo and loading bar
\item Change the weight calculation loop so that the inner loop range is constant, no matter \texttt{kde} 
\item Add Carbonetto-Stephens reference in documentation
\item Check how to refer to $\sigma^2$ (regression coefficient variance) and $\sigma^2_\beta$ (slab variance)  
\item Create a vignette demonstrating usage on a simple simulated dataset
\item Model details in return
...
\bibitem{covDepGM} (1) Dasgupta S., Ghosh P., Pati D., Mallick B. ``An approximate Bayesian approach to covariate dependent graphical modeling." 2021
\end{thebibliography}
