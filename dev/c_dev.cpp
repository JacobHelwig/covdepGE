...
// [[Rcpp::export]]
arma::mat test(bool monitor_elbo, int max_iter, int monitor_period){

  // instantiate the elbo_history matrix
  arma::mat elbo_history;

  // if elbo is to be monitored, construct the elbo_history matrix
  if (monitor_elbo){

    // create a vector for indexing the iteration at which ELBO is recorded
    arma::rowvec elbo_idx = arma::regspace<arma::rowvec>(1, monitor_period, max_iter);

    // create the matrix for storing ELBO
    elbo_history = arma::mat(2, elbo_idx.n_elem);

    // in the first row, put the iteration number
    elbo_history.row(0) = elbo_idx;
  }

    int stopping_time = 32;

    for (int j = 0; j < max_iter; j++){

      // if the iteration aligns with the monitor period, record the elbo
      if ((j % monitor_period == 0) & monitor_elbo){
        elbo_history(1, j / monitor_period) = 2 * (j + 1);
      }
    ...
    // remove the excess columns from elbo_history and add the value at the last
    // iteration, if it is not already in elbo_history
    if (monitor_elbo){

      // remove excess entries
      elbo_history = elbo_history.cols(arma::find(elbo_history.row(0) <= stopping_time));

      // assess whether the elbo value at the last iteration is in elbo_history
      if(stopping_time % monitor_period != 0){

        // if not add another column to elbo_history with the elbo at the last
        // iteration
        elbo_history = arma::join_rows(
          elbo_history, (arma::colvec){(double)stopping_time, -1});
      }
    }
