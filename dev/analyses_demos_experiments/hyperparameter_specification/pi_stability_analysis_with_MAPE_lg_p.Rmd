---
title: "pi stability analysis, large p"
output: pdf_document
toc: true
---
...
This analysis is the same as the one titled `pi_stability_analysis`, however, it considers the case with a large `p`; in this expeiment, `p + 1 = 25`. 

This experiment was to analyze the new method of hyperparameter selection with a focus placed on the selection of the optimal $\pi$. 100 trials were performed. In each trial, for each variable, $\pi$ was selected by maximizing ELBO over the following grid:
...
Here is an example of the output from a model fit to data generated from the precision matrices created as a function of the covariate in the section `Extraneous Covariate`.

The function returns the number of iterations performed in the last grid search. Notice that for most/ all variables, this is either 0 or 1000. The 0 can be explained by warm starts; recall that the value of $\alpha$ corresponding to the optimal $\pi$ in the previous grid search is used as the initial value in the last one. Thus, if the previous grid search converged, the last one will converge immediately as well. 

It also seems that if the previous grid search did not converge for a sufficiently large number of iterations, neither will the last grid search.

```{r, fig.width = 4.5, fig.height = 3.5}
out <- covdepGE(X, Z, max_iter = 1000, parallel = T, num_workers = 12, stop_cluster = F, R = T)

out

sapply(out$CAVI_details, `[[`, "iterations")

plot(out, colors, title_sum = T)
```
...
for (j in 1:n_trials){
  ...
  out_dep <- covdepGE(X, Z, max_iter = 1000, parallel = T, stop_cluster = F, warnings = F)
