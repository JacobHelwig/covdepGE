...
pip <- c(1e-5, 1e-4, 1e-3, 0.01, 0.02, 0.03, 0.04, seq(0.05, 0.2, 0.05),
         seq(0.3, 0.9, 0.1), 0.99, 0.999, 0.9999, 0.99999)
sbsq <- ssq <- c(1e-5, 1e-4, 1e-3, 0.01, 0.02, 0.03, 0.04, seq(0.05, 0.2, 0.05),
                 seq(0.3, 0.9, 0.1), seq(1, 3, 0.5), 5, 10)
hp_grid_lg <- expand.grid(ssq = ssq, sbsq = sbsq, pip = pip)
hp_grid_sm <- expand.grid(ssq = ssq, sbsq = sbsq, pip = pip[1:11])
...
hp_grid <- list(small = hp_grid_sm, large = hp_grid_lg)
grid_mods <- list(small = NA, large = NA)

# different methods for hyperparameter spec
methods <- c("grid_search", "model_average", "hybrid")
...
res <- foreach(trial_ind = 1:n_trials, .packages = "covdepGE") %dorng%{
  ...
  # create a list for each of the methods
  method_list <- replicate(3, grid_mods, F)
  names(method_list) <- methods

  # iterate over each of the grid types and method types
  for (method in methods){
    for (grid_type in names(hp_grid)){

      # fit the model and add it to the list
      method_list[[method]][[grid_type]] <- tryCatch(
        covdepGE(X, Z,
                 ssq = hp_grid[[grid_type]]$ssq,
                 sbsq = hp_grid[[grid_type]]$sbsq,
                 pip = hp_grid[[grid_type]]$pip,
                 grid_search = method, prog_bar = F),
