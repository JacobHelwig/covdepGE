...
# get all of the grid models
grid_mods <- lapply(res, `[[`, "grid")

# recover the marginal hyperparameter grids
marg_grids <- apply(res[[1]]$grid$hyperparameters$`Variable 1`$grid[ , 1:3], 2, 
                    unique)
```
...
# get all of the hyperparameter details from grid_models
grid_hyp <- lapply(grid_mods, `[[`, "hyperparameters")

# get the optimal pi for each of the trials by variable
pi_opt <- lapply(grid_hyp, lapply, `[[`, "pip")
...
sbsq_opt <- lapply(grid_hyp, lapply, `[[`, "sbsq")
...
ssq_opt <- lapply(grid_hyp, lapply, `[[`, "ssq")
...
table(pi_df$pi, sbsq_df$sbsq)
ggplot() + geom_jitter(aes(pi_vec, sbsq_vec), shape = 21, width = 0.01, height = 0.005, fill = "#80000075")
```
