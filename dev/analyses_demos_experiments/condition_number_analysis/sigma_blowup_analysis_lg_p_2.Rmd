...
blown_cond_nums <- sapply(blown_mats, function(mat) kappa(crossprod(mat)))
unblown_cond_nums <- sapply(unblown_mats, function(mat) kappa(crossprod(mat)))
ggplot(data.frame(condition_number = blown_cond_nums),aes(condition_number)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) + coord_cartesian(xlim = c(6, 11))
...
blown_cond <- sapply(blown_wt_mat2, function(mat) kappa(crossprod(mat)))
unblown_cond <- sapply(unblown_wt_mat2, function(mat) kappa(crossprod(mat)))
ggplot(data.frame(condition_number = blown_cond), aes(condition_number)) +
  geom_histogram(color = "black", fill = "tomato3", binwidth = 1) + theme_bw() +
  ggtitle("Distribution of condition numbers for weighted matrices that blew up") +
  theme(plot.title = element_text(hjust = 0.5)) + coord_cartesian(xlim = c(5, 45))
summary(blown_cond)
ggplot(data.frame(condition_number = unblown_cond), aes(condition_number)) +
  geom_histogram(color = "black", fill = "forestgreen", binwidth = 1) + theme_bw() +
...
cond_num_indv_180 <- lapply(wt_mat_indv, sapply, function(mat) kappa(crossprod(mat)))
...
ggplot(data.frame(Individual = 1:n, median_condition_number = med_cond_num_180), aes(
  ...
  coord_cartesian(ylim = c(11.9, 18.6)) + scale_x_continuous(breaks = c(1, seq(10, 180, 10)), labels = c(1, seq(10, 180, 10)))
```
...
ggplot(data.frame(X = mean_blow_conds - mean_unblow_conds), aes(X)) +
  geom_histogram(binwidth = .25, color = "black", fill = "forestgreen") + theme_bw() +
...
ggplot(data.frame(blown_indvs_blow_conds2["individual171"]), aes(individual171)) +
  geom_histogram(binwidth = 1, color = "black", fill = "tomato3") + theme_bw() +
  ggtitle("Condition number for matrices that blew up, individual 171") +
  theme(plot.title = element_text(hjust = 0.5)) + coord_cartesian(xlim = c(9, 41))
ggplot(data.frame(blown_indvs_unblow_conds2["individual171"]), aes(individual171)) +
  geom_histogram(binwidth = 1, color = "black", fill = "forestgreen") + theme_bw() +
