...
covdepGE(data_mat, Z, parallel = T, n_param = 5)
```

## Automatic parallel backend registration

```{r}
covdepGE(data_mat, Z, parallel = T, num_workers = 7, stop_cluster = F, n_param = 5)
```
...
```{r}
# sequential
out_seq <- covdepGE(data_mat, Z, n_param = 200, CS = T)
out_seq

# parallel
out_par <- covdepGE(data_mat, Z, n_param = 200, parallel = T, 
                    num_workers = 6, CS = T)
out_par
```
...
# sequential
out_seq <- covdepGE(data_mat, Z, n_param = 5)
out_seq

# parallel
out_par <- covdepGE(data_mat, Z, n_param = 5, parallel = T, num_workers = 8)
out_par
...
```{r, fig.width = 11, fig.height = 7}
# sequential
out_seq <- covdepGE(data_mat, Z, n_param = 5, warnings = F)
out_seq

# parallel
out_par <- covdepGE(data_mat, Z, n_param = 5, parallel = T, num_workers = 16, warnings = F)
out_par

set.seed(4)
colors <- c(colors, sample(colors()[sapply(colors(), function(color) !(substr(color, 1, 4)) %in% c("grey", "gray"))], max(0, out_par$model_details$num_unique - length(colors))))
