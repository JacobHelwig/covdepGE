...
Suppose $\pmb{X}\in \mathbb{R}^{n\times p}$ is a data matrix of independent observations $\pmb{X} = \left(\pmb{x}_1,..., \pmb{x}_p\right)$, where, for $j\in1,...,p$:

$\hspace{4cm}\pmb{x}_j\sim\mathcal N(\mu_j, \Sigma_{j,j}), \pmb x_j\in \mathbb{R}^n\hspace{2cm}\pmb{X}\sim\mathcal N(\mu, \Sigma)$

The conditional dependence structure of $\pmb{x_1},...,\pmb{x_p}$ can be modeled as an undirected graph $\mathcal{G}$ such that:

$\hspace{6cm}\mathcal G_{i,j} = \mathbb I (\mathrm{Cov}(\pmb{x_i}, \pmb{x_j})\neq 0)$
...
-   Check how to refer to $\sigma^2$ (regression coefficient variance) and $\sigma^2_\beta$ (slab variance)  
