...
-   `update_sigmasq`, `update_sigmabetasq`, `sigmasq_vec`, and `pi_vec` to `checks`

-   `sigmasq_vec` to `sigmasq` and `sigmabetasq_vec` to `sigmabetasq` in `covdepGE_main`; need to update this verbage
...
-   Remove `CS`, `R` and `resp_index` arguments

-   Center `X`

-   Fix hyperparameter grid size in print out
