...
Here we demonstrate an application of `covdepGE` to real-world data similar to that which was used in (1). In both cases, the protein expression values for genes from patients with Breast Invasive Carcinoma (BRCA) are modeled, with data sourced from The Cancer Genome Atlas (TCGA). (1) considers the following genes :
...
```{r, fig.height=15, warning=F, message=F}
