...
knitr::opts_knit$set(root.dir = "~/TAMU/Research/An approximate Bayesian approach to covariate dependent/covdepGE/Development")
```
...
// _____________________________________________________________________________
//[[Rcpp::export]]
Rcpp::List cov_vsvb_c_(const arma::colvec& y, const arma::mat& D,
...
// _____________________________________________________________________________
// [[Rcpp::export]]
arma::colvec sigma_loop_c_(const arma::colvec& y, const arma::mat& D,
                          const arma::mat& X_mat, const arma::mat& mu_mat,
                          const arma::mat& alpha_mat, double sigmasq,
  ...
  for (int j = 0; j < n_sigma; j++){
    ...
    elbo_graph = cov_vsvb_c_(y, D, X_mat, mu_mat, alpha_mat, sigmasq,
...
# Rcpp::sourceCpp("~/TAMU/Research/An approximate Bayesian approach to covariate dependent/covdepGE/Development/c_dev.cpp")
...
for (j in 1:5){
  n_j <- n * j; p_j <- p * j;
  set.seed(1)
  X <- mvtnorm::rmvnorm(n_j, rep(0, p_j), diag(p_j))
  Z <- matrix(rnorm(n_j))
  print(paste0("n: ", n_j))
  print(paste0("p: ", p_j))
  print(microbenchmark::microbenchmark(covdepGE1(X, Z), covdepGE::covdepGE(X, Z, print_time = T), times = 5))
  cat("\n\n")
}
```
